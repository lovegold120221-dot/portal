...
vi.mock('@/lib/clerk-users-api', () => ({
  clerkUsers: {
    inviteUser: vi.fn().mockResolvedValue({ id: 'inv_1' }),
  },
}))
...
describe('UsersInviteDialog', () => {
  ...
  it('sends a real invitation when the form is submitted successfully', async () => {
    const onOpenChange = vi.fn()
    const { getByRole } = await render(
      <UsersInviteDialog open onOpenChange={onOpenChange} />
    )
    ...
    expect(clerkUsers.inviteUser).toHaveBeenCalledOnce()
    expect(clerkUsers.inviteUser).toHaveBeenCalledWith({
