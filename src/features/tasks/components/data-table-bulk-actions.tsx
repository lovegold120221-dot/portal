...
export function DataTableBulkActions<TData>({
  ...
  const handleBulkStatusChange = (status: string) => {
    const selectedTasks = selectedRows.map((row) => row.original as unknown as Task)
    for (const task of selectedTasks) {
      updateTask(task.id, { status })
       }
    toast.success(`Status updated to "${status}" for ${selectedTasks.length} task(s).`)
  ...
  const handleBulkPriorityChange = (priority: string) => {
    ...
    toast.success(`Priority updated to "${priority}" for ${selectedTasks.length} task(s).`)
    table.resetRowSelection()
      }
